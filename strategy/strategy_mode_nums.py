...
def search_stocks_by_volume(path):
    ...
    for file in file_list:
        ...
        # Make sure we have at least 11 days of data
        if len(df) < 11:
            continue

        # Get the volume of 10 days ago
        df['volume_10_days_ago'] = df['volume'].shift(10)
        
        # Find the dates where the volume is 75% less than the volume of 10 trading days ago
        target_dates = df[df['volume'] < 0.25 * df['volume_10_days_ago']]['date']
